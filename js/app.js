            ...
            scope.$watch('filePreview', function (filePreview) {
                if (filePreview && filePreview.name) {
                    FileReader.readAsDataUrl(filePreview).then(function (result) {
                        if(!result){
                          alert('Gambar terlalu besar,ukuran maksimal adalah 3MB.');
                        }else{
...
.config(function($stateProvider, $urlRouterProvider) {
 ...
 .state('dash.home', {
  ...
  .state('dash.detailpegawai', {
    url: '/pegawai/detail',
    views: {
      'tab-dash': {
        templateUrl: 'templates/pegawai/detail_pegawai.html'
      }
    }
  })
 ...
 .state('dash.laporan', {
    ...
    views: {
      'tab-dash': {
        templateUrl: 'templates/laporan/laporan.html'
      }
    }
  })
  ...
  .state('dash.pendapatan', {
    url: '/pendapatan',
    views: {
      'tab-dash': {
        templateUrl: 'templates/keuangan/pendapatan.html',
        controller:'HomeCtrl'
      }
    }
  })
